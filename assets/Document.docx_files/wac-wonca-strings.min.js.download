...
var WacWoncaStringsEnum;
var WacWoncaStringsArray;
if (!WacWoncaStringsEnum) {
    if (typeof window !== 'undefined' && window.g_NewStringsInfra === true) {
        WacWoncaStringsEnum = Object.keys(WacWoncaStrings).reduce(function (acc, key, index) {
            acc[key] = index;
            return acc;
        }, {});
        if (!WacWoncaStringsArray) {
            WacWoncaStringsArray = Object.values(WacWoncaStrings);
        }
    }
    else {
        WacWoncaStringsEnum = WacWoncaStrings;
    }
}
